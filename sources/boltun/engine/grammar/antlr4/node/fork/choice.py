...
class ChoiceNode(ForkNode):
    ...
    def add_child(self, node):
        self._internal_add_child(node)

    def add_children(self, nodes):
        self._internal_add_children(nodes)

    def _internal_add_child(self, node, keep_ws=False):
        if isinstance(node, ChoiceNode.Wrapper):
            self._internal_add_children(node.children, node.keep_ws)
        else:
            if not keep_ws:
                node = node.strip()
            super(ChoiceNode, self).add_child(node)

    def _internal_add_children(self, nodes, keep_ws=False):
        for node in nodes:
            self._internal_add_child(node, keep_ws)
